...
module.exports = (function() {
    ...
    /**Sa c de moi et peut etre sa marche pas du tout
        Bah Si !
     * Sets the content tag of the config.xml file.
     *
     * @param {string} content The content of the config.xml content tag.
     */
    Config.prototype.setContent = function(content) {
        // Find the content tag
        var contentTag = this._doc.find('./content');

        if(!contentTag) {
            // If no content tag exists, create one
            contentTag = new et.Element('content');

            // Add the content tag to the root
            this._root.append(contentTag);
        }

        // Set the src attribute of the tag
        contentTag.attrib.src = content;
    };
